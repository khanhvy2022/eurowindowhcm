...
const process = [
  ["GIAI ĐOẠN 1", "Tư vấn & khảo sát", "Lắng nghe nhu cầu, khảo sát hiện trường và tư vấn hệ giải pháp cửa, vách phù hợp công năng và ngân sách."],
  ["GIAI ĐOẠN 2", "Sản xuất & kiểm tra", "Lập hồ sơ kỹ thuật, triển khai sản xuất tại nhà máy theo hệ chuẩn Châu Âu và kiểm tra chất lượng từng lô sản phẩm."],
  ["GIAI ĐOẠN 3", "Lắp đặt & bàn giao", "Lắp đặt bởi đội ngũ kinh nghiệm, nghiệm thu từng hạng mục và bàn giao kèm chế độ bảo hành chính hãng."],
] as const;
...
const featured = [
  ...
  { title: "Tòa nhà Quốc hội", location: "Công trình quốc gia", image: "/eurowindow/constructions/mg-1220.jpg.webp" },
] as const;
...
export default function ProductsPage() {
  ...
  return (
    <div className="min-h-screen bg-[#071523] text-white">
      <Header />
      <main>
        <PageBanner title="SẢN PHẨM CỦA EUROWINDOW" crumb="Sản phẩm" bgImage="/eurowindow/cuanhom.jpg.webp" />

        <section className="pb-24 pt-16">
          <div className="mx-auto max-w-[1320px] px-5 sm:px-8">
            <div className="flex flex-wrap gap-3" role="tablist" aria-label="Chọn sản phẩm">
              {services.map((service) => (
                <button
                  ...
                  className="rounded-full border border-white/10 bg-white/5 px-6 py-3 text-xs font-bold uppercase tracking-wider text-[#D2D8E3] backdrop-blur-md transition hover:border-[#E2C275] hover:text-white aria-selected:border-[#E2C275] aria-selected:bg-[#E2C275] aria-selected:text-[#071523] aria-selected:shadow-lg"
                >
                  {service.tab}
                </button>
              ))}
            ...
            <div className="mt-12 grid items-stretch gap-10 md:grid-cols-2">
              <div className="glass-card relative overflow-hidden p-3 backdrop-blur-2xl">
                <div className="relative aspect-[16/10] overflow-hidden rounded-xl bg-[#102238]">
                  <img key={current.id} src={current.image} alt={current.title} className="h-full w-full object-cover" />
                  <div className="absolute inset-0 bg-gradient-to-t from-[#071523]/80 via-transparent to-transparent" />
                </div>
              </div>

              <div className="glass-card flex flex-col justify-center p-8 backdrop-blur-2xl">
                <h2 className="text-3xl font-extrabold uppercase tracking-tight text-white md:text-4xl">{current.title}</h2>
                <p className="mt-4 leading-relaxed text-[#D2D8E3] text-sm sm:text-base">{current.text}</p>
                <ul className="mt-6 space-y-3">
                  {current.features.map((feature) => (
                    <li key={feature} className="flex items-start gap-3 text-xs font-semibold leading-relaxed text-[#D2D8E3] sm:text-sm">
                ...
                <div className="mt-8 grid grid-cols-3 gap-6 border-t border-white/10 pt-6">
                  <div>
                    <p className="text-2xl font-extrabold text-[#E2C275]">99%</p>
                    <p className="mt-1 text-xs text-[#94A3B8]">Khách Hàng Hài Lòng</p>
                  </div>
                  <div>
                    <p className="text-2xl font-extrabold text-[#E2C275]">12+</p>
                    <p className="mt-1 text-xs text-[#94A3B8]">Hệ Sản Phẩm Châu Âu</p>
                  </div>
                  <div>
                    <p className="text-2xl font-extrabold text-[#E2C275]">25 Năm</p>
                    <p className="mt-1 text-xs text-[#94A3B8]">Bảo Hành Chính Hãng</p>
                  </div>
                </div>
                <div className="mt-8">
                  <Link href="#lien-he" className="btn-gold-luxury inline-flex items-center gap-2 px-6 py-3.5 text-xs uppercase tracking-widest">
                    XEM BẢNG GIÁ ƯU ĐÃI <ArrowRight className="h-4 w-4" />
                  </Link>
                </div>
              </div>
            </div>

            <div key={`${current.id}-detail`} className="mt-20 border-t border-white/10 pt-16">
              <div className="grid gap-12 md:grid-cols-2">
                <div className="glass-card p-8 backdrop-blur-2xl">
                  <p className="text-xs font-bold uppercase tracking-widest text-[#E2C275]">Giới thiệu sản phẩm</p>
                  <div className="mt-6 space-y-5 leading-relaxed text-[#D2D8E3] text-sm sm:text-base">
                ...
                <div className="glass-card p-8 backdrop-blur-2xl">
                  <p className="text-xs font-bold uppercase tracking-widest text-[#E2C275]">Chi tiết cấu tạo &amp; công nghệ</p>
                  <div className="mt-6 space-y-6">
                    {current.structure.map((item: Section) => (
                      <div key={item.title} className="border-l-2 border-[#E2C275] pl-5">
                        ...
                        <p className="mt-2 leading-relaxed text-[#D2D8E3] text-xs sm:text-sm">{item.text}</p>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              <div className="mt-16">
                <p className="mb-6 text-xs font-bold uppercase tracking-widest text-[#E2C275]">Tính năng &amp; ưu việt</p>
                <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
                  {current.advantages.map((item: Advantage) => (
                    <div key={item.title} className="glass-card glass-card-hover p-6">
                      <h3 className="flex items-start gap-3 font-bold text-white text-base">
                        <Check className="mt-1 h-4 w-4 shrink-0 text-[#E2C275]" strokeWidth={3} />
                        {item.title}
                      </h3>
                      <p className="mt-3 text-xs leading-relaxed text-[#D2D8E3]">{item.text}</p>
                    </div>
                  ))}
                </div>
              </div>

              <div className="glass-card mt-16 p-8 backdrop-blur-2xl">
                <p className="text-xs font-bold uppercase tracking-widest text-[#E2C275]">Các hệ sản phẩm chi tiết</p>
                <ul className="mt-6 grid gap-x-8 gap-y-4 sm:grid-cols-2 lg:grid-cols-3">
                  {current.systems.map((system) => (
                    <li key={system} className="flex items-start gap-3 text-xs font-semibold leading-relaxed text-[#D2D8E3] sm:text-sm">
                      <Check className="mt-1 h-4 w-4 shrink-0 text-[#E2C275]" strokeWidth={3} />
                      {system}
                    </li>
                  ))}
        ...
        <section className="relative overflow-hidden border-t border-white/10 bg-[#102238]/60 py-24">
          <div className="mx-auto max-w-[1320px] px-5 sm:px-8">
            <h2 className="text-3xl font-extrabold uppercase tracking-tight text-white md:text-4xl">QUY TRÌNH THI CÔNG &amp; LẮP ĐẶT</h2>
            <ol className="mt-14 grid gap-8 md:grid-cols-3">
              {process.map(([stage, title, text]) => (
                <li key={stage} className="glass-card glass-card-hover p-8">
                  <p className="text-xs font-bold uppercase tracking-widest text-[#E2C275]">{stage}</p>
                  <h3 className="mt-3 text-xl font-extrabold text-white">{title}</h3>
                  <p className="mt-4 leading-relaxed text-[#D2D8E3] text-sm">{text}</p>
                </li>
              ))}
            </ol>
          </div>
        ...
        <section className="border-t border-white/10 py-24">
          <div className="mx-auto max-w-[1320px] px-5 sm:px-8">
            <div className="mb-12 flex items-end justify-between gap-6">
              ...
              <Link href="/du-an" className="hidden items-center gap-2 text-xs font-bold uppercase tracking-widest text-[#E2C275] md:inline-flex hover:text-[#F0D18A]">Xem tất cả dự án <ArrowRight className="h-4 w-4" /></Link>
            </div>
            <div className="grid gap-8 sm:grid-cols-2 lg:grid-cols-3">
              {featured.map((project) => (
                <Link key={project.title} href="/du-an" className="glass-card glass-card-hover group flex flex-col overflow-hidden p-5">
                  ...
                  <div className="pt-5">
                    <h3 className="text-lg font-bold tracking-tight text-white transition group-hover:text-[#E2C275]">{project.title}</h3>
                    <p className="mt-2 text-xs text-[#94A3B8]">{project.location}</p>
                  </div>
                </Link>
              ))}
            </div>
